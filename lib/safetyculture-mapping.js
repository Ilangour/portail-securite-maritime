...
// (CONFIG.CATALOGUE). "AUTRE" n'a pas de correspondance fixe : le template
// SafetyCulture ne propose pas d'option "DP" dédiée, voir AUTRE_VAUT_DP et
// mapExerciceLabel ci-dessous pour son traitement.
const LABEL_TO_EXERCICE = {
  ...
  // SURETE : jamais observé dans les inspections réelles à ce jour (pas de
  // couverture confirmée par ce template) — pas de mapping ajouté à
  // l'aveugle, à compléter si un libellé correspondant apparaît un jour.
};

// Navires où le template SafetyCulture n'a pas d'option "DP" dédiée : les
// exercices DP y sont saisis sous "AUTRE" (confirmé le 19/08/2026 sur les
// observations de deux inspections "AUTRE" mentionnant explicitement DP1/HMI/
// "full DP" sur le navire équivalent d'Aurore). Volontairement limité aux
// navires où DP existe dans CONFIG.CATALOGUE.NAVIRES_DATA (Aurore, Meridian,
// Neptune) : un "AUTRE" sur un navire sans DP reste "AUTRE" tel quel plutôt
// que d'être reclassé à l'aveugle.
const AUTRE_VAUT_DP = new Set(['Aurore', 'Meridian', 'Neptune']);
...
// tout de même apparaître dans l'Historique, décision utilisateur). `navire`
// (id interne, ex: "Aurore") permet le cas particulier "AUTRE" -> DP.
function mapExerciceLabel(label, navire) {
  if (label === 'AUTRE' && AUTRE_VAUT_DP.has(navire)) return 'DP';
...
const safetycultureMapping = { SITE_TO_NAVIRE, LABEL_TO_EXERCICE, AUTRE_VAUT_DP, mapSiteToNavire, mapExerciceLabel, extractExerciseData, resolveReconciliation };
